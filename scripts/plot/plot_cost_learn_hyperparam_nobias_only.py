...
def main():
    ...
    labels = [rf"(a) $\eta$ = {rates[0]}", rf"(b) $\eta$ = {rates[1]}"]
...
#            elif i==1: ax.set_ylim(1.0e-4, 1.0e-2)
    ...
    axes[1][0].set_xlabel(r"$\alpha_{\rm sc}$")
    # --- 凡例を1つにまとめる ---
#    handles, labels = axes[0][0].get_legend_handles_labels(loc="center")
    axes[1][0].legend(loc="upper center")
    
    fig.supylabel(r"normalized cost gap $\Delta C_T/W_T$ w/o PP", x=0.0015)

    fig.savefig(SAVE_DIR / "pdf" / f"cost_lean_rates{rates[0]}_{rates[1]}.pdf")
    plt.show()
